...
class FirestoreDbServices {
  ...
  Future<bool> saveUser(AppUser AppUser) async {
    try {
      await firestore.collection("users").doc(AppUser.userID).set(AppUser.toMap());
  ...
  Future<AppUser> readUser(String? userID) async {
    if (userID != null) {
      ...
      Map<String, dynamic> _okunanUserBilgileriMap = _OkunanVeriler.data() as Map<String, dynamic>;
  ...
  Future<bool> uzmanKaydet(AppUser UzmanUser) async {
    try {
      await firestore.collection("uzmanUsers").doc(UzmanUser.userID).set(UzmanUser.toMap());
      return true;
    } catch (e) {
      return false;
    }
  ...
  Future<List<AppUser>> UzmanlariGetir() async {
    ...
    QuerySnapshot querySnapshot = await firestore.collection("uzmanUsers").get();
  ...
  Stream<List<Mesaj>> getMesages(String mesajGonderenID, String mesajAlanID) {
    ...
    return snapShot.map(
        (mesaListesi) => mesaListesi.docs.map((mesaj) => Mesaj.fromMap(mesaj.data())).toList());
  }

  Future<bool> saveMessage(Mesaj kaydedilecekMesaj) async {
    var mesajID = firestore.collection("konusmalar").doc().id;
    var _gonderenDocID = kaydedilecekMesaj.kimden! + "--" + kaydedilecekMesaj.kime!;
